...
MegaDAVID <- function(database = NULL, x_var = NULL, y_var = NULL, decimals = NULL,
...
'
  ...
  for(k in 1:length(SpecificSentences)) {
    FS[[k]] <- paste0(All_Reference[[k]][,ncol(All_Reference[[k]])], " <- ", SpecificSentences[[k]])
    names(FS[[k]]) <- All_Reference[[k]][,ncol(All_Reference[[k]])]
  }
