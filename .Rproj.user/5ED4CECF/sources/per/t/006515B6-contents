...
RScience.TestCorrelacionPearson <- function(base = NULL, columnas_seleccionadas = c(1:ncol(base)),
                                            alfa = 0.05,
                                            decimales = 4,
                                            corte = 0.001){
  ...
  frase_mayor <- "El valor p es _valor_p_.
                    ...
                    Las variables '_var1_' y '_var2_' son estadísticamente no están correlacionadas.
                    La correlación es estadísticamente igual a cero."
  ...
  frase_menor <- "El valor p es _valor_p_.
                    ...
                    Las variables '_var1_' y '_var2_' están estadísticamente correlacionadas.
                    La correlación es estadísticamente distinto de cero."
  ...
  armado_sentencia_general <- paste0("stats::cor.test(base_mod[,_the_var_],
  ...
  for(i in seq_along(mis_variables)){
    ...
    armado_general$cada_sentencia[i] <- gsub("_the_var_", reemplazo, armado_sentencia_general)
  }
  ...
  for(i in seq_along(cada_sentencia)){
    ...
    base_mod <- na.omit(base[names(test_normalidad_var)[i]])

    # Detealles variosQGIS
    vector_n_original[i] <- nrow(base)
    vector_na[i] <- sum(is.na(base[names(test_normalidad_var)[i]]))
